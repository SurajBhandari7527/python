'''Dictionary in Python is a collection of keys values, used to store data values like a map, which, unlike other data types which hold
only a single value as an element.
In some languages it is known as map or assosiative arrays.
dict = { 'name': 'nitish', 'age': 33, 'gender' : 'male' }
Here, elements of dictionary comes up with a pair like: name is a key and its value is nitish, age is a key and its value is 33, 
gender is a key and its value is male.
Characterstics:
 Mutable
Indexing has no meaning
 keys can't be duplicated
 keys can't be mutable items
'''